...
export function UpdateUserTasks(userId) {
    ...
    // TODO:  Do we need to commit this?  http://stackoverflow.com/questions/22656517/update-in-foreach-on-mongodb-shell
...
export function FindTaskDetailFromTask(task){
    retval = null;
    console.log("finding task detail for task ")
    console.log(task);
...
export function TimeDeltaToPrettyString(utcNow, utcLater) {
    ...
    if (timeDelta <= 0) {
        ...
        if (nMinutesRemaining === 0) {
            prettyString = "under 1 minute remaining";
        }
        if (nMinutesRemaining === 1) {
            prettyString = "about 1 minute remaining";            
        }
        else {
         prettyString = "about " + nMinutesRemaining + " minutes remaining";
        }
    }
