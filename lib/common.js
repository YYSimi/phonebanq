...
export function FindTaskFromUserTask(userTask) {
    return Tasks.findOne(userTask.task_id);
}
    ...
    if (task.task_detail_id && task.task_type){
        switch(task.task_type) {
            case PBTaskTypesEnum.phone:
                retval = PhoneTasks.findOne(task.task_detail_id);
                break;
            case PBTaskTypesEnum.freeform:
                retval = FreeformTasks.findOne(task.task_detail_id);
                break;
            default:
...
export function getWebsiteGroup() {
    return UserGroups.findOne({name: "Website"});
}
